...
private:
  ...
  void on_mouse_motion(const SDL_MouseMotionEvent &event) {
	...
	if (Game_Model::get().get_player(0)->m_controls.throw_ball)
		Game_Model::get().get_player(0)->throw_ball();
	
	else if (Game_Model::get().get_player(0)->m_controls.pack)
		Game_Model::get().get_player(0)->pack_ball();	

	else if (Game_Model::get().get_player(0)->m_controls.finish_pack)
		Game_Model::get().get_player(0)->finish_packing();	
	...
	void render()
	{
		...
		vr.set_2d(make_pair(Point2f(0.0f, 0.0f)
			, Point2f(screen_x_c, screen_y_c)), true);

		Game_Model::get().get_player(0)->render_hud();
...
class Bootstrap {
  class Gamestate_One_Initializer : public Gamestate_Zero_Initializer {
    virtual Gamestate_Base * operator()() {
      Window::set_title("Ocean Water Clean Up");
      ...
      return new Title_State<Play_State_Top, Instructions_State>("Ocean Water\nClean Up\n");
    }
  } m_goi;
